// -----
// xxxxxxxxxxxxxxxxxxxxxxxxx lineas modificadas respecto a la librería 'OneButton' oficial
...
OneButton::OneButton(int pin, int activeLow)
{
  ...
  if (activeLow) {
    ...
    pinMode( pin, INPUT); //_PULLUP ); //xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    pinMode(pin, INPUT); // ???????????????? Should it be INPUT_PULLUP?
  } // if
...
void OneButton::tick(void)
{
  ...
  int button_read = analogRead(_pin); //digitalRead(_pin); // current button signal xxxxxxxxxxxxxxxxxxxxxxx
    int buttonLevel;
  if (button_read > 100)   buttonLevel = _buttonReleased;
  else                     buttonLevel = _buttonPressed;
